...
> **⚠️ 開発ステータス：大幅な内部構造改修中 (WIP)**
> 現在、プロセス制御を従来の `PowerShellHost` から、より柔軟な `ConPTY` (Pseudo Console) ベースの実装へ完全移行中です。
> 
> **現在の状況:**
> - `src/Core/ConPtyProcess.cs` および `VTStripper.cs` を追加しました。
> - `Program.cs` は既に `ConPTY` を使用するように書き換わっています。
> - **重要:** 現在のブランチでは、コマンドの実行やマクロの待機処理などが**正常に動作しない可能性が高い**です。安定稼働が必要な場合は、過去の安定版リリース（タグ等）をご利用ください。
> 
> 正常動作へ向けた実装を鋭意進めています。進捗にご期待ください！
