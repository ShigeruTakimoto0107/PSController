...
try {
    $appKey = "HKLM:\SOFTWARE\Classes\Applications\PowerShellController.exe"
    ...
    $macroKey = "HKLM:\SOFTWARE\Classes\PowerShellControllerMacro"
    ...
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Classes\$ext" -Name "(Default)" -Value "PowerShellControllerMacro" -Force

    New-Item -Path "HKLM:\SOFTWARE\Classes\$ext\OpenWithProgids" -Force | Out-Null
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Classes\$ext\OpenWithProgids" -Name "PowerShellControllerMacro" -Value "" -Force
