...
#### コマンドの直接記述（推奨）
PSCに登録されていないコマンド（PowerShellコマンド、ps1スクリプトパスなど）は、マクロに直接記述するだけでPowerShellにそのまま送信されます。これを「Unknownコマンド」と呼びます。変数展開も自動で行われます。`sendln` と完全に同等の動作をしますが、**こちらの直接記述を推奨します。**
...
- 直後に `getvar` を続ける場合は `wait >` を挟まないでください。
...
- 引数をダブルクォートで囲まないでください。クォートごとPowerShellに送信されます。
- 直後に `getvar` を続ける場合は `wait >` を挟まないでください。
- 直接記述（Unknownコマンド）と完全に同等のため、通常は直接記述を推奨します。`sendln` はマクロの可読性を高めたい場合や、PSCコマンドとPowerShellコマンドを明示的に区別したい場合に使用してください。
...
- 直接記述または `sendln` の直後に `getvar` が続く場合も同様です。
...
- 直前に必ず直接記述（Unknownコマンド）または `sendln` でPowerShellにコマンドを送信してください。
...
#### 変数の展開
マクロ内では `%変数名%` の形式で変数を展開できます。直接記述・`sendln`・`print`・`if` など多くのコマンドで使用できます。
...
#### `include ファイルパス`
外部マクロファイルを静的に展開します。相対パスはexeファイルのディレクトリを基準とします。
...
2. 直接記述または `sendln` の直後に `getvar` が続く場合（`getvar` 自身がプロンプトを待つため、`wait >` を挟むとフリーズします）
